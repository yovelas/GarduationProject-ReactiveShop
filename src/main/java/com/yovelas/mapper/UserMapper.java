...
public interface UserMapper {
    ...
    @Select("SELECT c.user_contact_id, c.user_actual_name, c.user_phone, c.user_email, user_address, c.create_time\n" +
            "FROM users_contact c\n" +
            "WHERE user_id = #{userId};")
    @Results({
            @Result(property = "userContactId",column = "user_contact_id"),
            @Result(property = "userActualName",column = "user_actual_name"),
